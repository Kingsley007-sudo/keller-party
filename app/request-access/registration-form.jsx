...
export default function RegistrationForm({
  ...
  if (isSubmitted) {
    return (
      <div className={`success-card${resolvedClassName}`}>
        <p className="flow-label">Submission received</p>
        <h3>{successTitle}</h3>
        <p className="flow-intro">{successMessage}</p>
      </div>
    );
  }

  return (
    <form className={`request-form${resolvedClassName}`} onSubmit={handleSubmit} noValidate>
      <div className="form-header">
        <div>
          <p className="flow-label">{flowLabel}</p>
          <h3>{title}</h3>
        </div>
      </div>
            ...
            <div key={`guest-${index}`} className="guest-card">
              ...
              <label className="field">
                ...
                <input
                  ...
                  onChange={(event) => handleGuestChange(index, "fullName", event.target.value)}
                  placeholder="Guest full name"
                />
                {errors.guests?.[index]?.fullName ? (
                  <span className="field-error">{errors.guests[index].fullName}</span>
                ) : null}
              </label>
              ...
              <label className="field">
                ...
                {errors.guests?.[index]?.instagramName ? (
                  <span className="field-error">{errors.guests[index].instagramName}</span>
                ) : null}
              </label>
            </div>
          ))}
      ...
      {submissionError ? <p className="field-error form-error-banner">{submissionError}</p> : null}
